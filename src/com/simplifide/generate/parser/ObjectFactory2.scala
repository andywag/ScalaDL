...
object ObjectFactory2 {
  ...
  def AdderTrunc(lhs:Expression,rhs:Expression,negative:Boolean = false,fixed:MemoryModel.Fixed,internal:MemoryModel.Fixed)
    = factory.AdderTrunc(lhs,rhs,negative,fixed,internal)
  def AdderTruncClip(lhs:Expression,rhs:Expression,negative:Boolean = false,fixed:MemoryModel.Fixed, internal:MemoryModel.Fixed) =
    factory.AdderTruncClip(lhs,rhs,negative, fixed, internal)
  def AdderRound(lhs:Expression,rhs:Expression,negative:Boolean = false,fixed:MemoryModel.Fixed,internal:MemoryModel.Fixed)     =
    factory.AdderRound(lhs,rhs,negative,fixed,internal)
  def AdderRoundClip(lhs:Expression,rhs:Expression,negative:Boolean = false,fixed:MemoryModel.Fixed,internal:MemoryModel.Fixed) =
    factory.AdderRoundClip(lhs,rhs,negative,fixed,internal)
  // Multiplier
  def Mult(lhs:Expression,rhs:Expression)(implicit clk:ClockControl) =
    factory.Mult(lhs,rhs,MemoryModel.NoFixed,MemoryModel.NoFixed)(clk)
  def MultTrunc(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,internal:MemoryModel.Fixed)     =
    factory.MultTrunc(lhs,rhs,fixed,internal)
  def MultTruncClip(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,internal:MemoryModel.Fixed) =
    factory.MultTruncClip(lhs,rhs,fixed,internal)
  def MultRound(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,internal:MemoryModel.Fixed)     =
    factory.MultRound(lhs,rhs,fixed,internal)
  def MultRoundClip(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,internal:MemoryModel.Fixed) =
    factory.MultRoundClip(lhs,rhs,fixed,internal)
  // Division
  def Div(lhs:Expression,rhs:Expression)                            = factory.Div(lhs,rhs)
  def DivTrunc(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed)     = factory.DivTrunc(lhs,rhs,fixed)
  def DivTruncClip(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed) = factory.DivTruncClip(lhs,rhs,fixed)
  def DivRound(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed)     = factory.DivRound(lhs,rhs,fixed)
  def DivRoundClip(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed) = factory.DivRoundClip(lhs,rhs,fixed)
  // Rounding
  def Truncate(condition:Expression, fixed:MemoryModel.Fixed, internal:MemoryModel.Fixed)    =
    factory.Truncate(condition,fixed,internal)
  def TruncateClip(condition:Expression, fixed:MemoryModel.Fixed, internal:MemoryModel.Fixed) =
    factory.TruncateClip(condition,fixed,internal)
  def RoundInt(condition:Expression, fixed:MemoryModel.Fixed, internal:MemoryModel.Fixed)     =
    factory.RoundInt(condition,fixed,internal)
  def RoundClip(condition:Expression, fixed:MemoryModel.Fixed, internal:MemoryModel.Fixed)    =
    factory.RoundClip(condition,fixed,internal)
  // Relational Opeators
  ...
  // Signal
  def Signal(name:String, typ:SignalType = SignalType.SignalTypeImpl,fixed:MemoryModel.Fixed = MemoryModel.NoFixed)(arr:List[Int]) =
    factory.Signal(name, typ ,fixed)(arr.toList)

  def Constant(name:String = "",value:Double,fixed:MemoryModel.Fixed = MemoryModel.NoFixed) =

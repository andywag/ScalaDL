...
public class ModuleTopASTNode extends TopASTNode{
    ...
    private List<ReferenceItem<SystemVar>> convertToVar(List<ReferenceItem> inputVars) {
        List<ReferenceItem<SystemVar>> vars = new ArrayList<ReferenceItem<SystemVar>>();
        for (ReferenceItem inputVar : inputVars) {
            vars.add(((SystemVar)inputVar.getObject()).createReferenceItem());
        }
        return vars;
    }
    ...
    protected void checkVariableDefinitions(ParseContext context, InstanceModule instMod) {
        ...
        List<ReferenceItem<SystemVar>> vars = convertToVar(module.findPrefixItemList(new ModuleObjectBaseItem(""), ReferenceUtilities.REF_SYSTEMVAR));
        ...
        for (ReferenceItem<PortTop> port : ports) {
            ReferenceItem<? extends SystemVar> varRef = port.getObject().getLocalVarReference();

...
trait SignalTrait extends SimpleSegment{
  ...
  /** Create Slice is used for creating the variables in an array whereas slice is
    ...
    */
  def createSlice(index:Int):SignalTrait = this.copy(this.name + "_" + index)
  ...
  /** Returns all of the children associated with this vector. This method only works on the vector portion
    * of the operation */
  override def getChildren:List[SignalTrait] = List()
  /** Create a list of signal declarations for this signal. This will expand the vector into a larger set of signals */
  def copy(nam:String,optype:OpType=opType,fix:FixedType=fixed):SignalTrait = newSignal(nam,optype,fix)
...
object SignalTrait {
  ...
  class Signal(override val name:String,override val opType:OpType,override val fixed:FixedType) extends SignalTrait {
      ...
      override def newSignal(nam:String,optype:OpType,fix:FixedType):SignalTrait = new Signal(name,optype,fix)
      override def slice(index:Int):SignalTrait = new Signal(name + "_" + index,opType,fixed)
  }
}

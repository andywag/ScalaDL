...
class StateTransitionHtmlTable(override val filename:String,val model:StateModel) extends ExtraFile {
    ...
    val contents = HtmlUtilities.fullHtml(filename + " State Machine Transition Table",List(transitionContents)).toString()

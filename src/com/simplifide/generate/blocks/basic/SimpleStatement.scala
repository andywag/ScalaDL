...
abstract class SimpleStatement(val output:SimpleSegment, val input:SimpleSegment) extends SimpleSegment {
...
object SimpleStatement {
  ...
  class Assign(output:SimpleSegment,input:SimpleSegment) extends SimpleStatement(output,input) {
    ...
    override def createCode(writer:CodeWriter):SegmentReturn = {
      ...
      val segments:List[SimpleSegment] = outIn.map(x => new Assign(x._1,x._2))

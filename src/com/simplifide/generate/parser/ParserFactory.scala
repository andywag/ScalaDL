...
object ParserFactory {
  ...
  def AdderTrunc(lhs:Expression,rhs:Expression,negative:Boolean = false,fixed:MemoryModel.Fixed,internal:Option[MemoryModel.Fixed])
    = new com.simplifide.generate.parser.math.Adder.Truncation("",lhs,rhs,negative,fixed,internal)
  def AdderTruncClip(lhs:Expression,rhs:Expression,negative:Boolean = false,fixed:MemoryModel.Fixed,internal:Option[MemoryModel.Fixed])
    = new com.simplifide.generate.parser.math.Adder.TruncationClip("",lhs,rhs,negative,fixed,internal)
  def AdderRound(lhs:Expression,rhs:Expression,negative:Boolean = false,fixed:MemoryModel.Fixed,internal:Option[MemoryModel.Fixed])
    = new com.simplifide.generate.parser.math.Adder.Round("",lhs,rhs,negative,fixed,internal)
  def AdderRoundClip(lhs:Expression,rhs:Expression,negative:Boolean = false,fixed:MemoryModel.Fixed,internal:Option[MemoryModel.Fixed])
    = new com.simplifide.generate.parser.math.Adder.RoundClip("",lhs,rhs,negative,fixed,internal)
  // Rounding Operations
  def Truncate(condition:Expression, fixed:MemoryModel.Fixed)      = new Round.Truncate(condition,fixed)
  def TruncateClip(condition:Expression, fixed:MemoryModel.Fixed)  = new Round.TruncateClip(condition,fixed)
  def RoundInt(condition:Expression, fixed:MemoryModel.Fixed)      = new Round.RoundInt(condition,fixed)
  def RoundClip(condition:Expression, fixed:MemoryModel.Fixed)     = new Round.RoundClip(condition,fixed)
  // Multiplier Operation
  def Mult(lhs:Expression,rhs:Expression,negative:Boolean = false)                            = new Multiplier(lhs,rhs)
  def MultTrunc(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,negative:Boolean = false)     = new Multiplier.Truncation(lhs,rhs,fixed)
  def MultTruncClip(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,negative:Boolean = false) = new Multiplier.TruncationClip(lhs,rhs,fixed)
  def MultRound(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,negative:Boolean = false)     = new Multiplier.Round(lhs,rhs,fixed)
  def MultRoundClip(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,negative:Boolean = false) = new Multiplier.RoundClip(lhs,rhs,fixed)
  // Division Operation
  def Div(lhs:Expression,rhs:Expression,negative:Boolean = false)                            = new Division(lhs,rhs)
  def DivTrunc(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,negative:Boolean = false)     = new Division.Truncation(lhs,rhs,fixed)
  def DivTruncClip(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,negative:Boolean = false) = new Division.TruncationClip(lhs,rhs,fixed)
  def DivRound(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,negative:Boolean = false)     = new Division.Round(lhs,rhs,fixed)
  def DivRoundClip(lhs:Expression,rhs:Expression,fixed:MemoryModel.Fixed,negative:Boolean = false) = new Division.RoundClip(lhs,rhs,fixed)
  ...
  // Signal Creation
  def Signal(name:String, typ:SignalType = SignalType.SignalTypeImpl,fixed:MemoryModel.Fixed = MemoryModel.Fixed(1,0))(arr:List[Int]) =

...
public class HardwareModule extends TopModule<ModuleObject>  {
    
	private static final long serialVersionUID = -6140098489459299990L;

	/** Pointer to Instance Module */
    private ReferenceItem<? extends InstanceModule> instModRef;
    ...
    public List<ReferenceItem<ModInstanceConnect>> getInstantiations() {
        ...
        ArrayList<ReferenceItem<ModInstanceConnect>> ret = new ArrayList<ReferenceItem<ModInstanceConnect>>();
        for (ReferenceItem ref : refList) {
            ret.add(ref.getObject().createReferenceItem());
        }
        
        return ret;
    }
    
    public List<ModInstanceConnect> getRealInstances() {
        List<ModInstanceConnect> connections = new ArrayList<ModInstanceConnect>();
        for (ReferenceItem<ModInstanceConnect> connect : getInstantiations()) {
            connections.add(connect.getObject());
        }
        return connections;
    }
    ...
    public void cleanObject() {
        ArrayList<ReferenceItem<? extends ModuleObject>> ulist = this.getGenericSelfList();
    ...
    public ReferenceItem<? extends InstanceModule> getInstModRef() {
        return instModRef;
    }
    
    public void setInstModRef(ReferenceItem<? extends InstanceModule> instModRef) {
